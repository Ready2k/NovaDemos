...
import { WorkflowDefinition, WorkflowNode } from './graph-types';
...
/**
 * Session context maintained by Agent Core
 */
export interface SessionContext {
    ...
    inputTokens?: number;
    outputTokens?: number;
    toolCallCounts?: Map<string, number>; // Track tool call counts for circuit breaker
 ...
 */
export type AgentResponseType = 'text' | 'tool_call' | 'handoff' | 'error';
...
export interface HandoffContext {
    ...
    summary?: string;
    idvFailed?: boolean;  // Flag to indicate IDV verification failed
}

/**
 * Workflow update structure
 */
...
export class AgentCore {
    ...
    private personaConfig: PersonaConfig | null;
     ...
     */
    public async processUserMessage(sessionId: string, message: string): Promise<AgentResponse> {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return {
                type: 'error',
     ...
     */
    private detectToolType(toolName: string): 'handoff' | 'banking' | 'knowledge_base' | 'local' | 'unknown' {
        // Check handoff tools
        if (isHandoffTool(toolName)) {
            return 'handoff';
        }
     ...
     */
    private validateBankingInput(toolName: string, input: any): { valid: boolean; error?: string } {
        switch (toolName) {
     ...
     */
    private validateKnowledgeBaseInput(input: any): { valid: boolean; error?: string } {
     ...
     */
    private async executeBankingTool(
        sessionId: string,
        toolName: string,
        toolInput: any
    ): Promise<ToolResult> {
     ...
     */
    public requestHandoff(sessionId: string, targetAgent: string, context: any): HandoffRequest {
        const session = this.sessions.get(sessionId);
        if (!session) {
     ...
     */
    public updateSessionMemory(sessionId: string, memory: any): void {
        const session = this.sessions.get(sessionId);
        if (!session) {
            console.error(`[AgentCore:${this.agentId}] Session not found: ${sessionId}`);
            return;
     ...
     * Get session memory
     */
    public getSessionMemory(sessionId: string): any {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return {};
        }
     ...
     */
    public updateWorkflowState(sessionId: string, nodeId: string): WorkflowUpdate {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
     ...
     */
    public getSystemPrompt(sessionId: string, personaPrompt?: string): string {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return '';
        }
...
` : '';
     ...
     */
    public getPersonaConfig(): PersonaConfig | null {
     ...
     */
    public getWorkflowDefinition(): WorkflowDefinition | null {
     ...
     */
    public trackAssistantResponse(sessionId: string, response: string, metadata?: any): void {
        const session = this.sessions.get(sessionId);
        if (!session?.langfuseTrace) {
            return;
        }
     ...
     */
    public trackTokenUsage(sessionId: string, inputTokens: number, outputTokens: number): void {
        const session = this.sessions.get(sessionId);
        if (!session?.langfuseTrace) {
            return;
        }
     ...
     */
    public trackInterruption(sessionId: string, reason?: string): void {
        const session = this.sessions.get(sessionId);
        if (!session?.langfuseTrace) {
            return;
        }
     ...
     */
    public trackError(sessionId: string, error: string, context?: any): void {
        const session = this.sessions.get(sessionId);
        if (!session?.langfuseTrace) {
            return;
        }
     ...
     */
    public getLangfuseTrace(sessionId: string): LangfuseTraceClient | undefined {
        const session = this.sessions.get(sessionId);
