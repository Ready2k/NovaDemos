...
class GatewayRouter {
    ...
    async routeToAgent(request) {
        ...
        try {
            ...
            const memoryUpdateSuccess = await this.updateGatewayMemory(request.sessionId, request.context);
            ...
            const transferSuccess = await this.requestAgentTransfer(request.sessionId, request.targetAgentId, request.reason);
            if (!transferSuccess) {
     ...
     */
    async updateGatewayMemory(sessionId, context) {
        try {
            const response = await axios_1.default.post(`${this.gatewayUrl}/api/sessions/${sessionId}/memory`, {
                sessionId,
                memory: context,
                timestamp: Date.now()
            }, {
     ...
     */
    async updateMemoryFallback(sessionId, context) {
     ...
     */
    async requestAgentTransfer(sessionId, targetAgentId, reason) {
        try {
            const response = await axios_1.default.post(`${this.gatewayUrl}/api/sessions/${sessionId}/transfer`, {
                sessionId,
    ...
    async getSessionMemory(sessionId) {
        try {
            const response = await axios_1.default.get(`${this.gatewayUrl}/api/sessions/${sessionId}/memory`, {
                timeout: this.timeout,
                headers: {
                    'X-Agent-Id': this.agentId
                }
    ...
    async notifyStatusChange(status, details) {
        try {
            await axios_1.default.post(`${this.gatewayUrl}/api/agents/${this.agentId}/status`, {
     ...
     */
    async isAgentAvailable(targetAgentId) {
        try {
            const response = await axios_1.default.get(`${this.gatewayUrl}/api/agents/${targetAgentId}`, {
                timeout: this.timeout
            });
            if (response.status === 200 && response.data) {
    ...
    async getAvailableAgents() {
        try {
            const response = await axios_1.default.get(`${this.gatewayUrl}/api/agents`, {
                timeout: this.timeout
            });
