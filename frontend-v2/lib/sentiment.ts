 ...
 * Ported from legacy frontend/sentiment-lite.js
...
const AFINN: Record<string, number> = {
    ...
    'survivor': 2, 'sweet': 2, 'talent': 2, 'terrific': 4, 'thoughtful': 2,
...
export function analyzeSentiment(text: string): SentimentResult {
    ...
    tokens.forEach(token => {
        if (Object.prototype.hasOwnProperty.call(AFINN, token)) {
            const itemScore = AFINN[token];
            words.push(token);
    ...
    if (comparative > 0.2) label = 'Very Positive';
    if (comparative < -0.05) label = 'Negative';
    if (comparative < -0.2) label = 'Very Negative';
